...
SegmentTree.prototype.getTreeBottomLevel = getTreeBottomLevel;
SegmentTree.prototype.getElementaryIntervals = getElementaryIntervals;
...
function getElementaryIntervals(node, enterFn) {
  if (node === null) node=root;
  // callback when enter node
  if (node.right===null && node.left ===null && (node.segment.from!==node.segment.to)) {
    enterFn(node);
  } else {
   if (node.right!==null) getElementaryIntervals(node.right, enterFn);
   if (node.left!==null) getElementaryIntervals(node.left, enterFn);
  }
}
...
function getTreeBottomLevel() {
  let tree = _tree2Array(root);
